...
app.controller('uploadController', ['$scope', '$interval', function($scope, $interval) {
	$scope.uploadedText = "";
	...
	$scope.uploadResults = function() {
		var resultText = "";
		var lines = $scope.uploadedText.split('\n');
		for(var i=0;i<lines.length;i++){
			var tmpH, tmpHS, tmpA, tmpAS;	
			if(!lines[i].charAt(0).match(/\d/) && !lines[i+1].charAt(0).match(/\d/)) {
				console.debug(changeName(lines[i]));
				tmpH = changeName(lines[i]);
				tmpA = changeName(lines[i+1]);
				var tmp = lines[i+2].split(/\:/);
				tmpHS = tmp[0].trim();
				tmpAS = tmp[1].trim();
				resultText += tmpH + " " + tmpHS + " " + tmpA + " " + tmpAS + "\n";
			}
		}
		postData(url, resultText);
		console.debug(resultText);
	}
	function changeName(tmpName) {
		for(var i=0; i<$scope.teams.length; i++){
			if(tmpName === $scope.teams[i].name) {
				return $scope.teams[i].sname;
			}
		}
	}
